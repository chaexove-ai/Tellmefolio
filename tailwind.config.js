...
export default {
  ...
  theme: {
    extend: {
      ...
      //
      // 크기는 Tailwind 기본값과 같습니다. 줄 간격만 늘렸습니다
      // (기본 16/20 → 18/22).
      //
      // 그 사이의 경과: 화면이 작아 보여 14/16 까지 올렸다가 13/15 로
      // 내렸고, 결국 원래 크기로 돌아왔습니다. 원인이 글씨가 아니라
      // 브라우저 확대 배율이었습니다 — 크롬은 사이트마다 배율을 따로
      // 기억하는데 localhost 쪽이 100% 가 아니었습니다. 배율을 맞추니
      // 원래 값이 적당했습니다.
      //
      // 줄 간격만 남긴 이유: 한글은 획이 많아 같은 크기에서도 줄이
      // 붙으면 빽빽해 보입니다. 글자를 키우지 않고 읽기만 편하게 하는
      // 방법이라, 이건 배율과 무관하게 이득입니다.
      //
      // 예외 두 곳:
      //   - .badge (index.css) 12px 고정 — 알약이라 커지면 줄을 밀어냄
      //   - portfolio-templates/ 는 고정 px — PDF 로 나가는 문서라 앱
      //     화면과 같이 움직이면 안 됨
      fontSize: {
        xs: ["12px", "18px"],
        sm: ["14px", "22px"],
