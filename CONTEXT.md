...
**2026-09-26** — **화면 구조 정리**(예시 docs/mockups/navigation.html): 사이드바 홈 / 내 포트폴리오 / 직무 전환 / 커뮤니티 / 설정 + 맨 위 "＋ 새 포트폴리오"(모든 만들기 버튼이 같은 선택 창 `components/NewPortfolio.tsx` — 대화로 만들기 · 자료로 만들기 두 이름만). 편집기·내보내기·제출 기록에서도 "내 포트폴리오"에 불. 내 포트폴리오 = 포트폴리오 / 제출 기록 두 탭(`LibraryTabs`). 홈은 입력창 · 최근 포트폴리오 4개 · 이어서 할 일만(현황 띠·책장·최근 작업·다음으로 할 일 제거, `Bookshelf.tsx` 삭제). 위저드 자료함·초안을 sessionStorage 에 임시 저장(`lib/sessionState.ts`) — "원본 자료 수정"·새로고침에도 유지, 포트폴리오를 만들면 비움. 들어갈 곳 없던 목업 `/community/share`·`/community/stats` 삭제.
