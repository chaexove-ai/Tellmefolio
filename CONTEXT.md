...
`tailwind.config.js` 의 `fontSize` 에서 스케일 자체를 다시 정의했습니다.
**컴포넌트에 px 를 직접 박지 말고 아래 유틸리티를 쓰세요.**

```
text-xs    13px / 19px   설명·라벨·보조 문구  ← 최소 기준
text-sm    15px / 23px   본문·입력칸 (기본)
text-base  17px / 26px
text-lg    19px / 28px
text-xl    22px / 30px   페이지 제목
```

**13px 아래로 내려가지 않습니다.** 한글은 같은 px 에서 영문보다 작게
읽힙니다 — 획이 많아 글자 속 공간이 좁기 때문입니다. 영문 기준 12px 은
한글에서 10px 처럼 보입니다.

예외는 두 군데뿐이고, 둘 다 이유가 물리적입니다.

- **`.badge`** — 12px 고정. 알약 모양이라 글자가 커지면 좌우로 길어져
  한 줄에 들어가던 배지들이 줄을 밀어냅니다. 상태를 나타내는 짧은
  낱말만 들어갑니다. 배지에 `text-xs` 를 덧붙이지 마세요 — 예외를 둔
  의미가 없어집니다.
- **책등(`Bookshelf`)** — 책등 폭이 54px 이라 물리적으로 좁습니다.
  12~13px 을 씁니다.

그리고 **`portfolio-templates/` 는 이 스케일을 따르지 않습니다.** 그쪽은
PDF 로 나가는 문서라 `text-[12px]` 처럼 고정값으로 박아뒀습니다. 앱 화면이
커진다고 문서까지 같이 커지면 지금까지 만든 PDF 와 줄바꿈이 달라집니다.
템플릿에 글자를 추가할 때도 고정 px 로 적으세요.
